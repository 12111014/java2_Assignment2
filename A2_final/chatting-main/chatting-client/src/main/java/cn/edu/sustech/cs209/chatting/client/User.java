...
public class User {
        ...
        if (!input.isPresent()) {
            ...
            switch (response) {
                case "YES": {
                    ...
                    //get all client
                    threadForClient = new Thread(gc);
                    threadForClient.start();
                    //get message
                    threadForMessage = new Thread(gm);
                    threadForMessage.start();
                    return true;
                }
                case "NO0": {
    ...
    public void throwingError(String info) {
        Alert warningForError = new Alert(Alert.AlertType.ERROR);
        warningForError.setTitle("Error");
        warningForError.setHeaderText(null);
        warningForError.setContentText(info);
        warningForError.showAndWait();
